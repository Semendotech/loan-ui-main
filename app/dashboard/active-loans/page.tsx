...
function ActiveLoansTable() {
  ...
  const loadLoans = async (searchQuery: string = "", pageNumber = 0) => {
    ...
    try {
      ...
      setLoans(items);
      setTotalLoans((res as any)?.total ?? 0);
      setPage(pageNumber);
    } catch (error) {
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="text-sm text-gray-600">
          {query.trim() ? "No loans found matching your search" : "No active loans found"}
        </div>
      ) : (
        <>
              ...
              <tbody>
                {loans.map((loan) => (
                  <tr key={loan.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-sm">KES {loan.remaining_amount?.toLocaleString() || "—"}</td>
                    <td className="px-4 py-2 text-sm">{loan.due_date || "—"}</td>
